...
HRESULT UiaTextRangeBase::RuntimeClassInitialize(_In_ IUiaData* pData,
                                                 _In_ IRawElementProviderSimple* const pProvider,
                                                 _In_ const til::coord start,
                                                 _In_ const til::coord end,
...
void UiaTextRangeBase::Initialize(_In_ const UiaPoint point)
{
    ...
    else
    {
        ...
        row = clientPoint.y / static_cast<LONG>(currentFontSize.y) + viewport.top;
    }
    _start = { 0, row };
    _end = _start;
}
...
const til::coord UiaTextRangeBase::GetEndpoint(TextPatternRangeEndpoint endpoint) const noexcept
{
    switch (endpoint)
    {
...
// Routine Description:
// - sets the target endpoint to the given til::coord value
...
bool UiaTextRangeBase::SetEndpoint(TextPatternRangeEndpoint endpoint, const til::coord val) noexcept
{
...
try
{
    ...
    *ppRetVal = nullptr;
    ...
    std::optional<til::coord> resultFirstAnchor;
    std::optional<til::coord> resultSecondAnchor;
...
try
{
    ...
    *ppRetVal = nullptr;
    ...
    if (searchBackward)
    {
        ...
        // because Search operates with an inclusive til::coord
        searchAnchor = _end;
...
IFACEMETHODIMP UiaTextRangeBase::GetBoundingRectangles(_Outptr_result_maybenull_ SAFEARRAY** ppRetVal) noexcept
{
    ...
    *ppRetVal = nullptr;
    ...
    try
    {
        ...
        // startAnchor: the earliest til::coord we will get a bounding rect for
        ...
        // endAnchor: the latest til::coord we will get a bounding rect for
...
const til::coord UiaTextRangeBase::_getScreenFontSize() const noexcept
{
    ...
    coordRet.x = std::max<decltype(coordRet.x)>(coordRet.x, 1);
    coordRet.y = std::max<decltype(coordRet.y)>(coordRet.y, 1);
...
// Return Value:
// - The viewport height
const til::CoordType UiaTextRangeBase::_getViewportHeight(const til::small_rect viewport) const noexcept
{
    FAIL_FAST_IF(!(viewport.bottom >= viewport.top));
    // + 1 because til::coord is inclusive on both sides so subtracting top
...
const Viewport UiaTextRangeBase::_getOptimizedBufferSize() const noexcept
{
    ...
    // because we want the returned viewport to include this til::coord
...
const til::coord UiaTextRangeBase::_getDocumentEnd() const
{
...
bool UiaTextRangeBase::_tryMoveToWordStart(const TextBuffer& buffer, const til::coord documentEnd, til::coord& resultingPos) const
{
...
til::coord UiaTextRangeBase::_getInclusiveEnd() noexcept
{
    auto result{ _end };
