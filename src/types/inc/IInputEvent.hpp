...
public:
    ...
    MouseEvent(const til::coord position,
    ...
    void SetPosition(const til::coord position) noexcept;
...
public:
    WindowBufferSizeEvent(const WINDOW_BUFFER_SIZE_RECORD& record) :
        _size{ til::wrap_coord(record.dwSize) }
    {
    }

    WindowBufferSizeEvent(const til::coord size) :
        _size{ size }
    {
    }
    ...
    til::coord GetSize() const noexcept
    {
        return _size;
    }

    void SetSize(const til::coord size) noexcept;
