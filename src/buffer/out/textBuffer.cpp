...
TextBuffer::TextBuffer(const til::coord screenBufferSize,
...
{
    // initialize ROWs
    _storage.reserve(screenBufferSize.y);
...
{
    return gsl::narrow<til::CoordType>(_storage.size());
}
...
{
    const til::CoordType totalRows = TotalRowCount();

    // Rows are stored circularly, so the index you ask for is offset by the start position and mod the total of rows.
    const til::CoordType offsetIndex = (_firstRow + index) % totalRows;
...
TextBufferTextIterator TextBuffer::GetTextDataAt(const til::coord at) const
{
...
TextBufferCellIterator TextBuffer::GetCellDataAt(const til::coord at) const
{
...
// Return Value:
// - Read-only iterator of text data only.
TextBufferTextIterator TextBuffer::GetTextLineDataAt(const til::coord at) const
{
...
// Return Value:
// - Read-only iterator of cell data.
TextBufferCellIterator TextBuffer::GetCellLineDataAt(const til::coord at) const
{
    til::small_rect limit;
    limit.top = at.y;
    limit.bottom = at.y;
    limit.left = 0;
...
// Return Value:
// - Read-only iterator of text data only.
TextBufferTextIterator TextBuffer::GetTextDataAt(const til::coord at, const Viewport limit) const
{
...
// Return Value:
// - Read-only iterator of cell data.
TextBufferCellIterator TextBuffer::GetCellDataAt(const til::coord at, const Viewport limit) const
{
                                         ...
                                         std::optional<til::CoordType> limitRight)
{
    ...
    const Viewport paint = Viewport::FromDimensions(target, { gsl::narrow<til::CoordType>(written), 1 });
    _NotifyPaint(paint);
...
void TextBuffer::_AdjustWrapOnCurrentRow(const bool fSet)
{
    ...
    const til::CoordType uiCurrentRowOffset = GetCursor().GetPosition().y;
...
til::coord TextBuffer::GetLastNonSpaceCharacter(std::optional<const Microsoft::Console::Types::Viewport> viewOptional) const
{
    const auto viewport = viewOptional.has_value() ? viewOptional.value() : GetSize();

    til::coord coordEndOfText;
    ...
    coordEndOfText.x = gsl::narrow<til::CoordType>(currRow.GetCharRow().MeasureRight() - 1);
    ...
    while (fDoBackUp)
    {
        ...
        coordEndOfText.x = gsl::narrow<til::CoordType>(backupRow.GetCharRow().MeasureRight() - 1);
        fDoBackUp = (coordEndOfText.x < 0 && coordEndOfText.y > viewportTop);
    }

    // don't allow negative results
    coordEndOfText.x = std::max<decltype(coordEndOfText.x)>(coordEndOfText.x, 0);
    coordEndOfText.y = std::max<decltype(coordEndOfText.y)>(coordEndOfText.y, 0);
...
til::coord TextBuffer::_GetPreviousFromCursor() const
{
...
{
    _size = Viewport::FromDimensions({ 0, 0 }, { gsl::narrow<til::CoordType>(_storage.at(0).size()), gsl::narrow<til::CoordType>(_storage.size()) });
}
...
til::coord TextBuffer::ClampPositionWithinLine(const til::coord position) const
{
    const auto rightmostColumn = GetLineWidth(position.y) - 1;
    return { std::min<decltype(position.x)>(position.x, rightmostColumn), position.y };
}

til::coord TextBuffer::ScreenToBufferPosition(const til::coord position) const
{
...
til::coord TextBuffer::BufferToScreenPosition(const til::coord position) const
{
...
[[nodiscard]] NTSTATUS TextBuffer::ResizeTraditional(const til::coord newSize) noexcept
{
    ...
    try
    {
        ...
        while (_storage.size() > static_cast<til::CoordType>(newSize.y))
        {
            _storage.pop_back();
        }
        ...
        while (_storage.size() < static_cast<til::CoordType>(newSize.y))
        {
            _storage.emplace_back(static_cast<til::CoordType>(_storage.size()), newSize.x, attributes, this);
        }
...
const DelimiterClass TextBuffer::_GetDelimiterClassAt(const til::coord pos, const std::wstring_view wordDelimiters) const
{
    return GetRowByOffset(pos.y).GetCharRow().DelimiterClassAt(pos.x, wordDelimiters);
}

// Method Description:
// - Get the til::coord for the beginning of the word you are on
// Arguments:
// - target - a til::coord on the word you are currently on
...
// Return Value:
// - The til::coord for the first character on the "word" (inclusive)
const til::coord TextBuffer::GetWordStart(const til::coord target, const std::wstring_view wordDelimiters, bool accessibilityMode, std::optional<til::coord> limitOptional) const
{
...
// Method Description:
// - Helper method for GetWordStart(). Get the til::coord for the beginning of the word (accessibility definition) you are on
// Arguments:
// - target - a til::coord on the word you are currently on
// - wordDelimiters - what characters are we considering for the separation of words
// Return Value:
// - The til::coord for the first character on the current/previous READABLE "word" (inclusive)
const til::coord TextBuffer::_GetWordStartForAccessibility(const til::coord target, const std::wstring_view wordDelimiters) const
{
    auto result = target;
    const auto bufferSize = GetSize();
...
// Method Description:
// - Helper method for GetWordStart(). Get the til::coord for the beginning of the word (selection definition) you are on
// Arguments:
// - target - a til::coord on the word you are currently on
// - wordDelimiters - what characters are we considering for the separation of words
// Return Value:
// - The til::coord for the first character on the current word or delimiter run (stopped by the left margin)
const til::coord TextBuffer::_GetWordStartForSelection(const til::coord target, const std::wstring_view wordDelimiters) const
{
    auto result = target;
    const auto bufferSize = GetSize();
...
// Method Description:
// - Get the til::coord for the beginning of the NEXT word
// Arguments:
// - target - a til::coord on the word you are currently on
// - wordDelimiters - what characters are we considering for the separation of words
...
// - limitOptional - (optional) the last possible position in the buffer that can be explored. This can be used to improve performance.
// Return Value:
// - The til::coord for the last character on the "word" (inclusive)
const til::coord TextBuffer::GetWordEnd(const til::coord target, const std::wstring_view wordDelimiters, bool accessibilityMode, std::optional<til::coord> limitOptional) const
{
    // Consider a buffer with this text in it:
    // "  word   other  "
    // In selection (accessibilityMode = false),
...
// Method Description:
// - Helper method for GetWordEnd(). Get the til::coord for the beginning of the next READABLE word
// Arguments:
// - target - a til::coord on the word you are currently on
// - wordDelimiters - what characters are we considering for the separation of words
// - limit - the last "valid" position in the text buffer (to improve performance)
// Return Value:
// - The til::coord for the first character of the next readable "word". If no next word, return one past the end of the buffer
const til::coord TextBuffer::_GetWordEndForAccessibility(const til::coord target, const std::wstring_view wordDelimiters, const til::coord limit) const
{
    const auto bufferSize{ GetSize() };
    til::coord result{ target };
...
// Method Description:
// - Helper method for GetWordEnd(). Get the til::coord for the beginning of the NEXT word
// Arguments:
// - target - a til::coord on the word you are currently on
// - wordDelimiters - what characters are we considering for the separation of words
// Return Value:
// - The til::coord for the last character of the current word or delimiter run (stopped by right margin)
const til::coord TextBuffer::_GetWordEndForSelection(const til::coord target, const std::wstring_view wordDelimiters) const
{
    const auto bufferSize = GetSize();
...
// Arguments:
// - pos - a til::coord on the word you are currently on
// - wordDelimiters - what characters are we considering for the separation of words
// - limitOptional - (optional) the last possible position in the buffer that can be explored. This can be used to improve performance.
// Return Value:
// - true, if successfully updated pos. False, if we are unable to move (usually due to a buffer boundary)
// - pos - The til::coord for the first character on the "word" (inclusive)
bool TextBuffer::MoveToNextWord(til::coord& pos, const std::wstring_view wordDelimiters, std::optional<til::coord> limitOptional) const
{
...
// - true, if successfully updated pos. False, if we are unable to move (usually due to a buffer boundary)
// - pos - The til::coord for the first character on the "word" (inclusive)
bool TextBuffer::MoveToPreviousWord(til::coord& pos, std::wstring_view wordDelimiters) const
{
...
// - limitOptional - (optional) the last possible position in the buffer that can be explored. This can be used to improve performance.
// Return Value:
// - pos - The til::coord for the first cell of the current glyph (inclusive)
const til::coord TextBuffer::GetGlyphStart(const til::coord pos, std::optional<til::coord> limitOptional) const
{
    auto resultPos = pos;
    const auto bufferSize = GetSize();
    const auto limit{ limitOptional.value_or(bufferSize.EndExclusive()) };
...
// Return Value:
// - pos - The til::coord for the last cell of the current glyph (exclusive)
const til::coord TextBuffer::GetGlyphEnd(const til::coord pos, bool accessibilityMode, std::optional<til::coord> limitOptional) const
{
    auto resultPos = pos;
    const auto bufferSize = GetSize();
    const auto limit{ limitOptional.value_or(bufferSize.EndExclusive()) };
...
// - true, if successfully updated pos. False, if we are unable to move (usually due to a buffer boundary)
// - pos - The til::coord for the first cell of the current glyph (inclusive)
bool TextBuffer::MoveToNextGlyph(til::coord& pos, bool allowExclusiveEnd, std::optional<til::coord> limitOptional) const
{
    const auto bufferSize = GetSize();
    const auto limit{ limitOptional.value_or(bufferSize.EndExclusive()) };
...
// Return Value:
// - true, if successfully updated pos. False, if we are unable to move (usually due to a buffer boundary)
// - pos - The til::coord for the first cell of the previous glyph (inclusive)
bool TextBuffer::MoveToPreviousGlyph(til::coord& pos, std::optional<til::coord> limitOptional) const
{
    auto resultPos = pos;
    const auto bufferSize = GetSize();
    const auto limit{ limitOptional.value_or(bufferSize.EndExclusive()) };
...
// Return Value:
// - the delimiter class for the given char
const std::vector<til::small_rect> TextBuffer::GetTextRects(til::coord start, til::coord end, bool blockSelection, bool bufferCoordinates) const
{
    std::vector<til::small_rect> textRects;
...
void TextBuffer::_ExpandTextRow(til::small_rect& textRow) const
{
    const auto bufferSize = GetSize();

    // expand left side of rect
    til::coord targetPoint{ textRow.left, textRow.top };
...
const TextBuffer::TextAndColor TextBuffer::GetText(const bool includeCRLF,
                                                   ...
                                                   const std::vector<til::small_rect>& selectionRects,
                                                   ...
                                                   const bool formatWrappedRows) const
{
    ...
    for (til::CoordType i = 0; i < rows; i++)
    {
        ...
        selectionText.reserve(gsl::narrow<til::CoordType>(highlight.Width()) + 2); // + 2 for \r\n if we munged it
        if (copyTextColor)
        {
            selectionFgAttr.reserve(gsl::narrow<til::CoordType>(highlight.Width()) + 2);
            selectionBkAttr.reserve(gsl::narrow<til::CoordType>(highlight.Width()) + 2);
        }
                           ...
                           std::optional<std::reference_wrapper<PositionInformation>> positionInfo)
{
    ...
    for (auto iOldRow = 0; iOldRow < cOldRowsTotal; iOldRow++)
    {
        ...
        auto iRight = gsl::narrow_cast<til::CoordType>(charRow.MeasureRight());
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (iRight < cOldColsTotal && !row.WasWrapForced())
            {
                ...
                else
                {
                    ...
                    {
                        if (newBuffer.GetRowByOffset(gsl::narrow_cast<til::CoordType>(coordNewCursor.y) - 1).WasWrapForced())
                        {
                            hr = newBuffer.NewlineCursor() ? hr : E_OUTOFMEMORY;
                        }
                    }
...
PointTree TextBuffer::GetPatterns(const til::CoordType firstRow, const til::CoordType lastRow) const
{
    ...
    for (const auto& idAndPattern : _idsAndPatterns)
    {
        ...
        for (auto i = words_begin; i != words_end; ++i)
        {
            ...
            const til::point startCoord{ gsl::narrow<til::CoordType>(start % rowSize), gsl::narrow<til::CoordType>(start / rowSize) };
            const til::point endCoord{ gsl::narrow<til::CoordType>(end % rowSize), gsl::narrow<til::CoordType>(end / rowSize) };
