...
{
public:
    TextBuffer(const til::coord screenBufferSize,
    ...
    TextBufferCellIterator GetCellDataAt(const til::coord at) const;
    TextBufferCellIterator GetCellLineDataAt(const til::coord at) const;
    TextBufferCellIterator GetCellDataAt(const til::coord at, const Microsoft::Console::Types::Viewport limit) const;
    TextBufferTextIterator GetTextDataAt(const til::coord at) const;
    TextBufferTextIterator GetTextLineDataAt(const til::coord at) const;
    TextBufferTextIterator GetTextDataAt(const til::coord at, const Microsoft::Console::Types::Viewport limit) const;
    ...
    til::coord GetLastNonSpaceCharacter(std::optional<const Microsoft::Console::Types::Viewport> viewOptional = std::nullopt) const;
    ...
    til::coord ClampPositionWithinLine(const til::coord position) const;
    til::coord ScreenToBufferPosition(const til::coord position) const;
    til::coord BufferToScreenPosition(const til::coord position) const;

    void Reset();

    [[nodiscard]] HRESULT ResizeTraditional(const til::coord newSize) noexcept;
    ...
    const til::coord GetWordStart(const til::coord target, const std::wstring_view wordDelimiters, bool accessibilityMode = false, std::optional<til::coord> limitOptional = std::nullopt) const;
    const til::coord GetWordEnd(const til::coord target, const std::wstring_view wordDelimiters, bool accessibilityMode = false, std::optional<til::coord> limitOptional = std::nullopt) const;
    bool MoveToNextWord(til::coord& pos, const std::wstring_view wordDelimiters, std::optional<til::coord> limitOptional = std::nullopt) const;
    bool MoveToPreviousWord(til::coord& pos, const std::wstring_view wordDelimiters) const;

    const til::coord GetGlyphStart(const til::coord pos, std::optional<til::coord> limitOptional = std::nullopt) const;
    const til::coord GetGlyphEnd(const til::coord pos, bool accessibilityMode = false, std::optional<til::coord> limitOptional = std::nullopt) const;
    bool MoveToNextGlyph(til::coord& pos, bool allowBottomExclusive = false, std::optional<til::coord> limitOptional = std::nullopt) const;
    bool MoveToPreviousGlyph(til::coord& pos, std::optional<til::coord> limitOptional = std::nullopt) const;

    const std::vector<til::small_rect> GetTextRects(til::coord start, til::coord end, bool blockSelection, bool bufferCoordinates) const;
    ...
    const TextAndColor GetText(const bool includeCRLF,
                               ...
                               const std::vector<til::small_rect>& textRects,
...
private:
    ...
    til::coord _GetPreviousFromCursor() const;
    ...
    void _ExpandTextRow(til::small_rect& selectionRow) const;

    const DelimiterClass _GetDelimiterClassAt(const til::coord pos, const std::wstring_view wordDelimiters) const;
    const til::coord _GetWordStartForAccessibility(const til::coord target, const std::wstring_view wordDelimiters) const;
    const til::coord _GetWordStartForSelection(const til::coord target, const std::wstring_view wordDelimiters) const;
    const til::coord _GetWordEndForAccessibility(const til::coord target, const std::wstring_view wordDelimiters, const til::coord limit) const;
    const til::coord _GetWordEndForSelection(const til::coord target, const std::wstring_view wordDelimiters) const;
