...
public:
    ...
    til::coord GetPosition() const noexcept;
    ...
    void SetPosition(const til::coord cPosition) noexcept;
    ...
    void DelayEOLWrap(const til::coord coordDelayedAt) noexcept;
    void ResetDelayEOLWrap() noexcept;
    til::coord GetDelayedAtPosition() const noexcept;
...
private:
    ...
    til::coord _cPosition; // current position on screen (in screen buffer coords).
    ...
    til::coord _coordDelayedAt; // coordinate the EOL wrap was delayed at.
