...
til::coord Terminal::GetTextBufferEndPosition() const noexcept
{
    // We use the end line of mutableViewport as the end
    // of the text buffer, it always moves with the written
    // text
    til::coord endPosition{ _GetMutableViewport().Width() - 1, ViewEndIndex() };
    return endPosition;
}
...
til::coord Terminal::GetCursorPosition() const noexcept
{
...
const std::vector<size_t> Terminal::GetPatternId(const til::coord location) const noexcept
{
    // Look through our interval tree for this location
    const auto intervals = _patternIntervalTree.findOverlapping(til::coord{ location.x + 1, location.y }, location);
    if (intervals.size() == 0)
    {
        return {};
    }
...
void Terminal::SelectNewRegion(const til::coord coordStart, const til::coord coordEnd)
{
#pragma warning(push)
