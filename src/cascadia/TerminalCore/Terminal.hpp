...
public:
    ...
    void Create(til::coord viewportSize,
...
#pragma region ITerminalApi
    ...
    til::coord GetCursorPosition() noexcept override;
...
#pragma region ITerminalInput
    ...
    bool SendMouseEvent(const til::coord viewportPos, const unsigned int uiButton, const ControlKeyStates states, const short wheelDelta, const Microsoft::Console::VirtualTerminal::TerminalInput::MouseButtonState state) override;
    bool SendCharEvent(const wchar_t ch, const WORD scanCode, const ControlKeyStates states) override;

    [[nodiscard]] HRESULT UserResize(const til::coord viewportSize) noexcept override;
    ...
    std::wstring GetHyperlinkAtPosition(const til::coord position);
    uint16_t GetHyperlinkIdAtPosition(const til::coord position);
    std::optional<interval_tree::IntervalTree<til::point, size_t>::interval> GetHyperlinkIntervalFromPosition(const til::coord position);
#pragma endregion
...
#pragma region IUiaData
    ...
    const til::coord GetSelectionAnchor() const noexcept override;
    const til::coord GetSelectionEnd() const noexcept override;
    const std::wstring_view GetConsoleTitle() const noexcept override;
    void ColorSelection(const til::coord coordSelectionStart, const til::coord coordSelectionEnd, const TextAttribute) override;
...
#pragma region TextSelection
    ...
    void MultiClickSelection(const til::coord viewportPos, SelectionExpansion expansionMode);
    void SetSelectionAnchor(const til::coord position);
    void SetSelectionEnd(const til::coord position, std::optional<SelectionExpansion> newExpansionMode = std::nullopt);
...
#pragma region Text Selection
    ...
    // the pivot is the til::coord that remains selected when you extend a selection in any direction
...
#pragma endregion
    ...
    void _InvalidateFromCoords(const til::coord start, const til::coord end);
    ...
    void _AdjustCursorPosition(const til::coord proposedPosition);
...
#pragma region TextSelection
    // These methods are defined in TerminalSelection.cpp
    std::vector<til::small_rect> _GetSelectionRects() const noexcept;
    std::pair<til::coord, til::coord> _PivotSelection(const til::coord targetPos, bool& targetStart) const;
    std::pair<til::coord, til::coord> _ExpandSelectionAnchors(std::pair<til::coord, til::coord> anchors) const;
    til::coord _ConvertToBufferCell(const til::point& viewportPos) const;
    void _MoveByChar(SelectionDirection direction, til::coord& pos);
    void _MoveByWord(SelectionDirection direction, til::coord& pos);
    void _MoveByViewport(SelectionDirection direction, til::coord& pos);
    void _MoveByBuffer(SelectionDirection direction, til::coord& pos);
#pragma endregion
