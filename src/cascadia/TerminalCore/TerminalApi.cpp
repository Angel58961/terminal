...
try
{
    ...
    til::coord newPos{ absoluteX, absoluteY };
...
til::coord Terminal::GetCursorPosition() noexcept
{
    ...
    til::coord newPos{ relativeX, relativeY };
...
try
{
    ...
    const til::coord copyFromPos{ cursorPos.x + dist, cursorPos.y };
...
try
{
    ...
    const til::coord copyToPos{ cursorPos.x + dist, cursorPos.y };
    const auto sourceWidth = _mutableViewport.RightExclusive() - copyFromPos.x;
    SHORT width;
    if (!SUCCEEDED(UIntToShort(sourceWidth, &width)))
    {
...
try
{
    ...
    const auto fillLimit = std::min(static_cast<short>(numChars), distanceToRight);
