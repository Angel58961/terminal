...
namespace TerminalCoreUnitTests
{
    class SelectionTest
    {
        ...
        // Return Value:
        // - N/A
        void ValidateSingleRowSelection(Terminal& term, til::small_rect expected)
        {
        ...
        TEST_METHOD(SelectArea)
        {
            ...
            for (auto selectionRect : selectionRects)
            {
                ...
                {
                    // Verify top line
                    VERIFY_ARE_EQUAL(selection, til::small_rect({ 5, 10, rightBoundary, 10 }));
                }
                else if (rowValue == 20)
                {
                    // Verify bottom line
                    VERIFY_ARE_EQUAL(selection, til::small_rect({ 0, 20, 15, 20 }));
                }
                else
                {
                    // Verify other lines (full)
                    VERIFY_ARE_EQUAL(selection, til::small_rect({ 0, rowValue, rightBoundary, rowValue }));
                }
        ...
        {
            const til::coord maxCoord{ SHRT_MAX, SHRT_MAX };

            // Test SetSelectionAnchor(til::coord) and SetSelectionEnd(til::coord)
            // Behavior: clamp coord to viewport.
            auto ValidateSingleClickSelection = [&](SHORT scrollback, til::small_rect expected) {
                Terminal term;
                DummyRenderTarget emptyRT;
                term.Create({ 10, 10 }, scrollback, emptyRT);

                // NOTE: SetSelectionEnd(til::coord) is called within SetSelectionAnchor(til::coord)
            ...
            auto ValidateDoubleClickSelection = [&](SHORT scrollback, til::small_rect expected) {
                Terminal term;
                DummyRenderTarget emptyRT;
                term.Create({ 10, 10 }, scrollback, emptyRT);
            ...
            auto ValidateTripleClickSelection = [&](SHORT scrollback, til::small_rect expected) {
                Terminal term;
                DummyRenderTarget emptyRT;
                term.Create({ 10, 10 }, scrollback, emptyRT);
        ...
        TEST_METHOD(SelectToOutOfBounds)
        {
            ...
            */
            ...
            ValidateSingleRowSelection(term, til::small_rect({ 5, 5, rightBoundary, 5 }));
            ...
            term.SetSelectionEnd({ 5, -20 });
            {
                ...
                for (auto selectionRect : selectionRects)
                {
                    ...
                    {
                        // Verify top line
                        VERIFY_ARE_EQUAL(selection, til::small_rect({ 5, rowValue, rightBoundary, rowValue }));
                    }
                    else if (rowValue == 5)
                    {
                        // Verify last line
                        VERIFY_ARE_EQUAL(selection, til::small_rect({ leftBoundary, rowValue, 5, rowValue }));
                    }
                    else
                    {
                        // Verify other lines (full)
                        VERIFY_ARE_EQUAL(selection, til::small_rect({ leftBoundary, rowValue, rightBoundary, rowValue }));
                    }
                }
            }
            ...
            term.SetSelectionEnd({ 5, 20 });
            {
                ...
                for (auto selectionRect : selectionRects)
                {
                    ...
                    {
                        // Verify top line
                        VERIFY_ARE_EQUAL(selection, til::small_rect({ 5, 5, rightBoundary, 5 }));
                    }
                    else if (rowValue == 9)
                    {
                        // Verify bottom line
        ...
        TEST_METHOD(SelectBoxArea)
        {
            ...
            for (auto selectionRect : selectionRects)
            {
                ...
                // Verify all lines
                VERIFY_ARE_EQUAL(selection, til::small_rect({ 5, rowValue, 15, rowValue }));
        ...
        TEST_METHOD(SelectWideGlyphsInBoxSelection)
        {
            ...
            for (auto selectionRect : selectionRects)
            {
                ...
                if (rowValue == 10)
                {
                    VERIFY_ARE_EQUAL(selection, til::small_rect({ 4, rowValue, 7, rowValue }));
                }
                else if (rowValue == 11)
                {
                    VERIFY_ARE_EQUAL(selection, til::small_rect({ 5, rowValue, 8, rowValue }));
                }
                else
                {
                    // Verify all lines
                    VERIFY_ARE_EQUAL(selection, til::small_rect({ 5, rowValue, 7, rowValue }));
                }
        ...
        TEST_METHOD(DoubleClick_GeneralCase)
        {
            ...
            // Validate selection area
            ValidateSingleRowSelection(term, til::small_rect({ 4, 10, (4 + text.size() - 1), 10 }));
        }
        ...
        TEST_METHOD(DoubleClick_Delimiter)
        {
            ...
            // Validate selection area
            ValidateSingleRowSelection(term, til::small_rect({ 0, 10, 99, 10 }));
        }
        ...
        TEST_METHOD(DoubleClick_DelimiterClass)
        {
            ...
            ValidateSingleRowSelection(term, til::small_rect({ 15, 10, 15, 10 }));
        }
        ...
        TEST_METHOD(DoubleClickDrag_Right)
        {
            ...
            // Validate selection area
            ValidateSingleRowSelection(term, til::small_rect({ 4, 10, 32, 10 }));
        }
        ...
        TEST_METHOD(TripleClickDrag_Vertical)
        {
            ...
            VERIFY_ARE_EQUAL(selection, til::small_rect({ 0, 10, 99, 10 }));

            // verify second selection rect
            selection = term.GetViewport().ConvertToOrigin(selectionRects.at(1)).ToInclusive();
            VERIFY_ARE_EQUAL(selection, til::small_rect({ 0, 11, 99, 11 }));
        }
        ...
        TEST_METHOD(ShiftClick)
        {
            ...
            // Step 1: Create a selection on "doubleClickMe"
            {
                ...
                ValidateSingleRowSelection(term, til::small_rect({ 4, 10, 16, 10 }));
            }
            ...
            // Step 2: Shift+Click to "dragThroughHere"
            {
                ...
                ValidateSingleRowSelection(term, til::small_rect({ 4, 10, 21, 10 }));
            }
            ...
            // Step 4: Shift+Triple-Click at "dragThroughHere"
            {
                ...
                // Validate selection area: "doubleClickMe dragThroughHere..." selected
                ValidateSingleRowSelection(term, til::small_rect({ 4, 10, 99, 10 }));
            }
        ...
        TEST_METHOD(Pivot)
        {
            ...
            // Step 1: Create a selection
            {
                ...
                // Validate selection area
                ValidateSingleRowSelection(term, til::small_rect({ 10, 10, 20, 10 }));
            }
            ...
            // Step 2: Drag to (5,10)
            {
                ...
                ValidateSingleRowSelection(term, til::small_rect({ 5, 10, 10, 10 }));
            }
