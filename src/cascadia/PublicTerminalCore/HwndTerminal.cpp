...
HRESULT HwndTerminal::Initialize()
{
    ...
    const til::coord windowSize{ windowRect.right - windowRect.left, windowRect.bottom - windowRect.top };
    ...
    _terminal->Create(til::coord{ 80, 25 }, 1000, *_renderer);
...
HRESULT HwndTerminal::Refresh(const SIZE windowSize, _Out_ til::coord* dimensions)
{
...
HRESULT _stdcall TerminalTriggerResize(_In_ void* terminal, _In_ short width, _In_ short height, _Out_ til::coord* dimensions)
{
    const auto publicTerminal = static_cast<HwndTerminal*>(terminal);
...
/// <returns>HRESULT of the attempted resize.</returns>
HRESULT _stdcall TerminalTriggerResizeWithDimension(_In_ void* terminal, _In_ til::coord dimensionsInCharacters, _Out_ SIZE* dimensionsInPixels)
{
...
HRESULT _stdcall TerminalCalculateResize(_In_ void* terminal, _In_ short width, _In_ short height, _Out_ til::coord* dimensions)
{
    const auto publicTerminal = static_cast<const HwndTerminal*>(terminal);
...
til::coord HwndTerminal::GetFontSize() const noexcept
{
...
void HwndTerminal::ChangeViewport(const til::small_rect NewWindow)
{
