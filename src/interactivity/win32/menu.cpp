...
[[nodiscard]] HRESULT Menu::s_GetConsoleState(CONSOLE_STATE_INFO* const pStateInfo)
{
    ...
    pStateInfo->ScreenBufferSize = til::unwrap_coord(ScreenInfo.GetBufferSize().Dimensions());
    pStateInfo->WindowSize = til::unwrap_coord(ScreenInfo.GetViewport().Dimensions());
    ...
    pStateInfo->FontSize = til::unwrap_coord(currentFont.GetUnscaledSize());
...
void Menu::s_PropertiesUpdate(PCONSOLE_STATE_INFO pStateInfo)
{
    ...
    FontInfo fiNewFont(pStateInfo->FaceName, gsl::narrow_cast<unsigned char>(pStateInfo->FontFamily), pStateInfo->FontWeight, til::wrap_coord(pStateInfo->FontSize), pStateInfo->CodePage);
...

    {
        ...
        auto coordWindow = til::wrap_coord(pStateInfo->WindowSize);

        // Requested buffer in characters.
        auto coordBuffer = til::wrap_coord(pStateInfo->ScreenBufferSize);
