...
public:
    ...
    void ChangeViewport(const til::small_rect /*NewWindow*/)
    {
        return;
    }
...
class UiaTextRangeTests
{
    ...
    TEST_METHOD(ExpandToEnclosingUnit)
    {
        ...
        auto verifyExpansion = [&](TextUnit textUnit, til::coord utrStart, til::coord utrEnd) {
        ...
        for (auto textUnit : supportedUnits)
        {
            ...
            if (textUnit != TextUnit_Character)
            {
                ...
                const til::coord end{ boundaries.start.x + 1, boundaries.start.y };
            ...
            if (textUnit != TextUnit_Character && textUnit != TextUnit_Document)
            {
                ...
                const til::coord end{ boundaries.end.x + 1, boundaries.end.y };
                verifyExpansion(textUnit, boundaries.start, end);
            }
            ...
            if (textUnit != TextUnit_Character)
            {
                ...
                const til::coord start{ boundaries.start.x + 1, boundaries.start.y };
            ...
            if (textUnit != TextUnit_Character)
            {
                ...
                const til::coord start{ boundaries.start.x + 1, boundaries.start.y };
                const til::coord end{ start.x + 1, start.y };
    ...
    {
        const til::coord start{ 0, 1 };
        const til::coord end{ 1, 2 };
        Microsoft::WRL::ComPtr<UiaTextRange> utr;
        THROW_IF_FAILED(Microsoft::WRL::MakeAndInitialize<UiaTextRange>(&utr,
                                                                        _pUiaData,
                                                                        &_dummyProvider,
        ...
        Log::Comment(L"Move target's end to utr1's start");
        {
            ...
            VERIFY_ARE_EQUAL(target->GetEndpoint(TextPatternRangeEndpoint_Start), origin.to_vec<til::coord>());
    ...
    TEST_METHOD(CanMoveByCharacter)
    {
        ...
        const til::coord documentEnd{ _pTextBuffer->GetSize().Left(), _pTextBuffer->GetLastNonSpaceCharacter(.y + 1) };
    ...
    TEST_METHOD(CanMoveEndpointByUnitCharacter)
    {
        ...
        // GH#6986: This is used as the "end of the buffer" to help screen readers run faster
        //          instead of parsing through thousands of empty lines of text.
        const til::coord documentEnd{ _pTextBuffer->GetSize().RightInclusive(), _pTextBuffer->GetLastNonSpaceCharacter().y };
    ...
    TEST_METHOD(ExpansionAtExclusiveEnd)
    {
        ...
        const til::coord documentEnd{ 0, lastNonspaceCharPos.y + 1 };
        ...
        THROW_IF_FAILED(Microsoft::WRL::MakeAndInitialize<UiaTextRange>(&utr, _pUiaData, &_dummyProvider, bufferEnd.to_vec<til::coord>(), endExclusive.to_vec<til::coord>()));
    ...
    TEST_METHOD(MovementAtExclusiveEnd)
    {
        ...
        // write "temp" at (2,2)
        _pTextBuffer->Reset();
        const til::coord writeTarget{ 2, 2 };
        _pTextBuffer->Write({ L"temp" }, writeTarget);

        // GH#6986: This is used as the "end of the buffer" to help screen readers run faster
        //          instead of parsing through thousands of empty lines of text.
        const til::coord documentEndInclusive{ base::ClampSub<short, short>(static_cast<short>(bufferSize.right()), 1), _pTextBuffer->GetLastNonSpaceCharacter().y };
        const til::coord documentEndExclusive{ static_cast<short>(bufferSize.left()), base::ClampAdd(documentEndInclusive.y, 1) };

        const til::coord lastLineStart{ static_cast<short>(bufferSize.left()), documentEndInclusive.y };
        const auto secondToLastLinePos{ point_offset_by_line(lastLineStart, bufferSize, -1) };
        const til::coord secondToLastCharacterPos{ documentEndInclusive.x - 1, documentEndInclusive.y };
        ...
        if (degenerate)
        {
            ...
            THROW_IF_FAILED(Microsoft::WRL::MakeAndInitialize<UiaTextRange>(&utr, _pUiaData, &_dummyProvider, utrStart.to_vec<til::coord>(), utrEnd.to_vec<til::coord>()));
        }
        else
        {
    ...
    TEST_METHOD(MoveToPreviousWord)
    {
        ...
        _pTextBuffer->Write({ L"My name is Carlos" }, origin.to_vec<til::coord>());

        // Create degenerate UTR at origin
        Microsoft::WRL::ComPtr<UiaTextRange> utr;
        THROW_IF_FAILED(Microsoft::WRL::MakeAndInitialize<UiaTextRange>(&utr, _pUiaData, &_dummyProvider, origin.to_vec<til::coord>(), origin.to_vec<til::coord>()));
        ...
        const til::coord expectedStart{ 3, 0 };
    ...
    TEST_METHOD(ScrollIntoView)
    {
        ...
        for (const auto test : testData)
        {
            ...
            THROW_IF_FAILED(Microsoft::WRL::MakeAndInitialize<UiaTextRange>(&utr, _pUiaData, &_dummyProvider, pos.to_vec<til::coord>(), pos.to_vec<til::coord>()));
    ...
    {
        Microsoft::WRL::ComPtr<UiaTextRange> utr;
        const til::coord startPos{ 0, 0 };
        const til::coord endPos{ 0, 2 };
    ...
    TEST_METHOD(Movement)
    {
        ...
        {
            auto i = 0;
            auto iter{ _pTextBuffer->GetCellDataAt(origin.to_vec<til::coord>()) };
            const auto segment{ bufferSize.width() / 5 };
            while (iter.Pos() != documentEnd.to_vec<til::coord>())
            {
                bool fill{ true };
                if (i % segment == 0)
                {
    ...
    TEST_METHOD(GeneratedMovementTests)
    {
        ...
        {
            auto i = 0;
            auto iter{ _pTextBuffer->GetCellDataAt(bufferSize.origin().to_vec<til::coord>()) };
            const auto segment{ bufferSize.width() / 10 };
            bool fill{ true };
            while (iter.Pos() != docEnd.to_vec<til::coord>())
            {
        ...
        else
        {
            Microsoft::WRL::ComPtr<UiaTextRange> utr;
            int amountMoved;
            THROW_IF_FAILED(Microsoft::WRL::MakeAndInitialize<UiaTextRange>(&utr, _pUiaData, &_dummyProvider, testCase.input.start.to_vec<til::coord>(), testCase.input.end.to_vec<til::coord>()));
