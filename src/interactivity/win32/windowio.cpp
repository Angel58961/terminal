...
bool HandleTerminalMouseEvent(const til::coord cMousePosition,
    ...
    if (pSelection->IsInSelectingState() || pSelection->IsInQuickEditMode())
    {
        ...
        else if (Message == WM_LBUTTONDBLCLK)
        {
            ...
            if (MousePosition == selectionAnchor)
            {
                try
                {
                    const std::pair<til::coord, til::coord> wordBounds = ScreenInfo.GetWordBoundary(MousePosition);
