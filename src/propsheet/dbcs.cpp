    ...
    __out_ecount(LF_FACESIZE) LPTSTR AltFaceName)
{
    ...
    for (i = 0; i < NumberOfFonts; i++)
    {
        ...
        {
            FontDelta.x = (SHORT)abs(gsl::narrow_cast<int>(FontSize.x - FontInfo[i].Size.x));
            FontDelta.y = (SHORT)abs(gsl::narrow_cast<int>(FontSize.y - FontInfo[i].Size.y));
