...
bool IsValidSmallRect(_In_ til::small_rect* const Rect)
{
    ...
    for (unsigned int i = 0; i < pIme->ConvAreaCompStr.size(); ++i)
    {
        ...
        if (!ConvAreaInfo.IsHidden())
        {
            ...
            if (IsValidSmallRect(&ClippedRegion))
            {
                ...
                using RectType = decltype(ClippedRegion.left);
                ClippedRegion.left = std::max<RectType>(Region.left, convRegion.Left());
                ClippedRegion.top = std::max<RectType>(Region.top, convRegion.Top());
                ClippedRegion.right = std::min<RectType>(Region.right, convRegion.RightInclusive());
                ClippedRegion.bottom = std::min<RectType>(Region.bottom, convRegion.BottomInclusive());
                if (IsValidSmallRect(&ClippedRegion))
                {
...
[[nodiscard]] HRESULT ConsoleImeResizeCompStrScreenBuffer(const til::coord coordNewScreenSize)
{
    CONSOLE_INFORMATION& gci = ServiceLocator::LocateGlobals().getConsoleInformation();
    ConsoleImeInfo* const pIme = &gci.ConsoleIme;
