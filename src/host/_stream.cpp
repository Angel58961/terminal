                                            ...
                                            _Inout_opt_ til::CoordType* psScrollY)
{
    ...
    if (scrollDownAtTop)
    {
        ...
        const til::coord newPostMarginsOrigin{ 0, moveToYPosition };
        const til::coord newViewOrigin{ 0, newViewTop };
...
#pragma prefast(suppress : 26019, "Buffer is taken in multiples of 2. Validation is ok.")
    ...
    EndWhile:
        if (i != 0)
        {
            ...
            i = std::min<decltype(i)>(i, coordScreenBufferSize.x - CursorPosition.x);
