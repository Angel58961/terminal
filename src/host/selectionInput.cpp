...
// Return Value:
// - <none>
til::coord Selection::WordByWordSelection(const bool fReverse,
                                     ...
                                     const til::coord coordSelPoint) const
{
    ...
    til::coord coordMaxLeft;
    til::coord coordMaxRight;
...
bool Selection::HandleKeyboardLineSelectionEvent(const INPUT_KEY_INFO* const pInputKeyInfo)
{
    ...
    til::coord coordInputLineStart;
    til::coord coordInputLineEnd;
        ...
        if (fShiftPressed)
        {
            try
            {
                ...
                if (selectionRects.size() > 0)
                {
                    ...
                    {
                        auto it = screenInfo.GetCellDataAt(til::coord{ selectRect.left, selectRect.top });
...
[[nodiscard]] bool Selection::s_GetInputLineBoundaries(_Out_opt_ til::coord* const pcoordInputStart, _Out_opt_ til::coord* const pcoordInputEnd)
{
    const CONSOLE_INFORMATION& gci = ServiceLocator::LocateGlobals().getConsoleInformation();
    const auto bufferSize = gci.GetActiveOutputBuffer().GetBufferSize();
...
// Return Value:
// - If true, the boundaries returned are valid. If false, they should be discarded.
void Selection::GetValidAreaBoundaries(_Out_opt_ til::coord* const pcoordValidStart, _Out_opt_ til::coord* const pcoordValidEnd) const
{
    const CONSOLE_INFORMATION& gci = ServiceLocator::LocateGlobals().getConsoleInformation();
    til::coord coordEnd;
    coordEnd.x = 0;
...
bool Selection::s_IsWithinBoundaries(const til::coord coordPosition, const til::coord coordStart, const til::coord coordEnd)
{
