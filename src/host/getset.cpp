                                                   ...
                                                   CONSOLE_SCREEN_BUFFER_INFOEX& data) noexcept
{
    try
    {
        ...
        auto dwSize = til::wrap_coord(data.dwSize);
        auto dwCursorPosition = til::wrap_coord(data.dwCursorPosition);
        auto dwMaximumWindowSize = til::wrap_coord(data.dwMaximumWindowSize);
        ...
        data.dwSize = til::unwrap_coord(dwSize);
        data.dwCursorPosition = til::unwrap_coord(dwCursorPosition);
        data.dwMaximumWindowSize = til::unwrap_coord(dwMaximumWindowSize);
    }
    CATCH_LOG();
}
                                                          ...
                                                          COORD& size) noexcept
{
    try
    {
        ...
        if (index == 0)
        {
            ...
            size = til::unwrap_coord(context.GetActiveBuffer().GetCurrentFont().GetUnscaledSize());
            return S_OK;
        }
        else
        {
                                                               ...
                                                               CONSOLE_FONT_INFOEX& consoleFontInfoEx) noexcept
{
    try
    {
        ...
        consoleFontInfoEx.dwFontSize = til::unwrap_coord(WindowSize);
                                                               ...
                                                               const CONSOLE_FONT_INFOEX& consoleFontInfoEx) noexcept
{
    try
    {
        ...
        FontInfo fi(FaceName,
                    ...
                    til::wrap_coord(consoleFontInfoEx.dwFontSize),
                    gci.OutputCP);
                                                  ...
                                                  COORD& size) noexcept
{
    try
    {
        ...
        size = til::unwrap_coord(screenInfo.GetLargestWindowSizeInCharacters());
    }
    CATCH_LOG();
}
                                                                    ...
                                                                    const CONSOLE_SCREEN_BUFFER_INFOEX& data) noexcept
{
    try
    {
            ...
            requestedBufferSize.Y != coordScreenBufferSize.y)
        {
            ...
            LOG_IF_FAILED(context.ResizeScreenBuffer(til::wrap_coord(data.dwSize), TRUE));
...
[[nodiscard]] HRESULT ApiRoutines::SetConsoleCursorPositionImpl(SCREEN_INFORMATION& context,
                                                                const til::coord position) noexcept
{
    try
    {
        LockConsole();
...
[[nodiscard]] HRESULT ApiRoutines::SetConsoleWindowInfoImpl(SCREEN_INFORMATION& context,
                                                            ...
                                                            const til::small_rect& windowRect) noexcept
{
    try
    {
        LockConsole();
                                                             ...
                                                             COORD& newSize) noexcept
{
    try
    {
        ...
        LockConsole();
        {
            ...
            newSize = til::unwrap_coord(screenInfo.GetBufferSize().Dimensions());
...
[[nodiscard]] NTSTATUS DoSrvPrivateReverseLineFeed(SCREEN_INFORMATION& screenInfo)
{
    ...
    til::coord newCursorPosition{ oldCursorPosition.x, oldCursorPosition.y - 1 };
