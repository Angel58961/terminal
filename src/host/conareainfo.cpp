...
ConversionAreaBufferInfo::ConversionAreaBufferInfo(const til::coord coordBufferSize) :
...
ConversionAreaInfo::ConversionAreaInfo(const til::coord bufferSize,
                                       const til::coord windowSize,
...
[[nodiscard]] HRESULT ConversionAreaInfo::Resize(const til::coord newSize) noexcept
{
    ...
    using RectType = decltype(_caInfo.rcViewCaWindow.left);
    const RectType restrictionX{ newSize.x - 1 };
    const RectType restrictionY{ newSize.y - 1 };
    _caInfo.rcViewCaWindow.left = std::min(_caInfo.rcViewCaWindow.left, restrictionX);
    _caInfo.rcViewCaWindow.right = std::min(_caInfo.rcViewCaWindow.right, restrictionX);
    _caInfo.rcViewCaWindow.top = std::min(_caInfo.rcViewCaWindow.top, restrictionY);
    _caInfo.rcViewCaWindow.bottom = std::min(_caInfo.rcViewCaWindow.bottom, restrictionY);

    return S_OK;
}

void ConversionAreaInfo::SetWindowInfo(const til::small_rect view) noexcept
{
...
void ConversionAreaInfo::SetViewPos(const til::coord pos) noexcept
{
    if (IsHidden())
    {
