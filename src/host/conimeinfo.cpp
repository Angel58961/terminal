...
[[nodiscard]] HRESULT ConsoleImeInfo::ResizeAllAreas(const til::coord newSize)
{
    for (auto& area : ConvAreaCompStr)
    {
        if (!area.IsHidden())
                                                                             ...
                                                                             SCREEN_INFORMATION& screenInfo)
{
    ...
    const til::small_rect region{ insertionPos.x, 0, gsl::narrow<til::CoordType>(insertionPos.x + lineVec.size() - 1), 0 };
    ...
    {
        screenInfo.NotifyAccessibilityEventing(insertionPos.x, insertionPos.y, gsl::narrow<til::CoordType>(insertionPos.x + lineVec.size() - 1), insertionPos.y);
    }
