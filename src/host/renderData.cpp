...
// Return Value:
// - til::coord of the end position of the text buffer
til::coord RenderData::GetTextBufferEndPosition() const noexcept
{
    const CONSOLE_INFORMATION& gci = ServiceLocator::LocateGlobals().getConsoleInformation();
    Viewport bufferSize = gci.GetActiveOutputBuffer().GetBufferSize();
    til::coord endPosition{ bufferSize.Width() - 1, bufferSize.BottomInclusive() };
    return endPosition;
}
...
til::coord RenderData::GetCursorPosition() const noexcept
{
    const CONSOLE_INFORMATION& gci = ServiceLocator::LocateGlobals().getConsoleInformation();
...
const std::vector<size_t> RenderData::GetPatternId(const til::coord /*location*/) const noexcept
{
    return {};
}
...
// Return Value:
// - <none>
void RenderData::SelectNewRegion(const til::coord coordStart, const til::coord coordEnd)
{
...
const til::coord RenderData::GetSelectionAnchor() const noexcept
{
...
// Return Value:
// - current selection anchor
const til::coord RenderData::GetSelectionEnd() const noexcept
{
...
void RenderData::ColorSelection(const til::coord coordSelectionStart, const til::coord coordSelectionEnd, const TextAttribute attr)
{
