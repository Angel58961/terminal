...
{
public:
    [[nodiscard]] static NTSTATUS CreateInstance(_In_ til::coord coordWindowSize,
    ...
    void GetScreenBufferInformation(_Out_ til::coord* pcoordSize,
    ...
    void ClipToScreenBuffer(_Inout_ til::small_rect* const psrClip) const;

    til::coord GetMinWindowSizeInCharacters(const til::coord coordFontSize = { 1, 1 }) const;
    til::coord GetMaxWindowSizeInCharacters(const til::coord coordFontSize = { 1, 1 }) const;
    til::coord GetLargestWindowSizeInCharacters(const til::coord coordFontSize = { 1, 1 }) const;
    til::coord GetScrollBarSizesInCharacters() const;

    Microsoft::Console::Types::Viewport GetBufferSize() const;
    Microsoft::Console::Types::Viewport GetTerminalBufferSize() const;

    til::coord GetScreenFontSize() const;
    void UpdateFont(const FontInfo* const pfiNewFont);
    void RefreshFontWithRenderer();

    [[nodiscard]] NTSTATUS ResizeScreenBuffer(const til::coord coordNewScreenSize, const bool fDoScrollBarUpdate);
    ...
    void SetViewportSize(const til::coord* const pcoordSize);

    // Forwarders to Window if we're the active buffer.
    [[nodiscard]] NTSTATUS SetViewportOrigin(const bool fAbsolute, const til::coord coordWindowOrigin, const bool updateBottom);
    ...
    std::pair<til::coord, til::coord> GetWordBoundary(const til::coord position) const;
...
public:
    ...
    [[nodiscard]] NTSTATUS SetCursorPosition(const til::coord Position, const bool TurnOn);

    void MakeCursorVisible(const til::coord CursorPosition, const bool updateBottom = true);
    ...
    bool IsCursorInMargins(const til::coord cursorPosition) const noexcept;
...
private:
    ...
    void _CalculateViewportSize(const RECT* const prcClientArea, _Out_ til::coord* const pcoordSize);
    void _AdjustViewportSize(const RECT* const prcClientNew, const RECT* const prcClientOld, const til::coord* const pcoordSize);
    void _InternalSetViewportSize(const til::coord* const pcoordSize, const bool fResizeFromTop, const bool fResizeFromLeft);
    ...
    [[nodiscard]] NTSTATUS ResizeWithReflow(const til::coord coordnewScreenSize);
    [[nodiscard]] NTSTATUS ResizeTraditional(const til::coord coordNewScreenSize);
