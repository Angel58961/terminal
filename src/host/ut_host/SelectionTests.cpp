...
class SelectionTests
{
    ...
    void VerifyGetSelectionRects_BoxMode()
    {
        ...
        if (VERIFY_ARE_EQUAL(cRectanglesExpected, selectionRects.size()))
        {
            for (auto iRect = 0; iRect < gsl::narrow<int>(selectionRects.size()); iRect++)
            {
                ...
                const til::small_rect* const psrRect = &selectionRects[iRect];

                const auto sRectangleLineNumber = (short)iRect + m_pSelection->_srSelectionRect.top;
    ...
    void VerifyGetSelectionRects_LineMode()
    {
        ...
        if (VERIFY_ARE_EQUAL(cRectanglesExpected, selectionRects.size()))
        {
            ...
            if (fHaveOneLine)
            {
                ...
                const til::small_rect* const psrRect = &selectionRects[0];
            ...
            else
            {
                ...
                const til::small_rect* const psrFirst = &selectionRects[iFirst];
                const til::small_rect* const psrLast = &selectionRects[iLast];
    ...
    void TestBisectSelectionDelta(SHORT sTargetX, SHORT sTargetY, SHORT sLength, SHORT sDeltaLeft, SHORT sDeltaRight)
    {
        ...
        short sStringLength;
        til::coord coordTargetPoint;
        til::small_rect srSelection;
        ...
        til::coord startPos{ sTargetX, sTargetY };
        til::coord endPos{ base::ClampAdd(sTargetX, sLength), sTargetY };
...
class SelectionInputTests
{
    ...
    TEST_METHOD(TestGetInputLineBoundaries)
    {
        ...
        til::coord coordFinalPos;

        const auto cCharsToAdjust = ((short)readData.VisibleCharCount() - 1); // then -1 to be on the last piece of text, not past it
    ...
    TEST_METHOD(TestWordByWordPrevious)
    {
        ...
        const til::coord left;
        const til::coord right{ text.length(), 0 };
