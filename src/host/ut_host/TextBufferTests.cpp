...
void TextBufferTests::ResizeTraditionalRotationPreservesHighUnicode()
{
    ...
    const til::coord bufferSize{ 80, 10 };
    ...
    const til::coord newPos{ pos.x, pos.y + delta };
...
void TextBufferTests::ResizeTraditionalHighUnicodeRowRemoval()
{
    ...
    const til::coord pos{ 0, bufferSize.y - 1 };
    auto position = _buffer->_storage[pos.y].GetCharRow().GlyphAt(pos.x);
    ...
    til::coord trimmedBufferSize{ bufferSize.x, bufferSize.y - 1 };
...
void TextBufferTests::ResizeTraditionalHighUnicodeColumnRemoval()
{
    ...
    const til::coord pos{ bufferSize.x - 1, 0 };
    auto position = _buffer->_storage[pos.y].GetCharRow().GlyphAt(pos.x);
    ...
    til::coord trimmedBufferSize{ bufferSize.x - 1, bufferSize.y };
...
void TextBufferTests::GetWordBoundaries()
{
    ...
    // Test Data:
    // - til::coord - starting position
    // - til::coord - expected result (accessibilityMode = false)
    // - til::coord - expected result (accessibilityMode = true)
    struct ExpectedResult
    {
        til::coord accessibilityModeDisabled;
        til::coord accessibilityModeEnabled;
    };
    ...
    {
        Log::Comment(NoThrowString().Format(L"til::coord (%hd, %hd)", test.startPos.x, test.startPos.y));
...
void TextBufferTests::MoveByWord()
{
    ...
    // Test Data:
    // - til::coord - starting position
    // - til::coord - expected result (moving forwards)
    // - til::coord - expected result (moving backwards)
    struct ExpectedResult
    {
        til::coord moveForwards;
        til::coord moveBackwards;
    };
...
void TextBufferTests::GetTextRects()
{
    ...
    std::vector<til::small_rect> expected{};
    if (blockSelection)
    {
