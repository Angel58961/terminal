...
class ScreenBufferTests
{
    ...
    TEST_METHOD_SETUP(MethodSetup)
    {
        ...
        auto defaultSize = til::coord{ CommonState::s_csWindowWidth, CommonState::s_csWindowHeight };
        currentBuffer.SetViewport(Viewport::FromDimensions(defaultSize), true);
        VERIFY_ARE_EQUAL(til::coord({ 0, 0 }), currentBuffer.GetTextBuffer().GetCursor().GetPosition());
...
void ScreenBufferTests::AlternateBufferCursorInheritanceTest()
{
    ...
    auto mainCursorPos = til::coord{ 3, 5 };
    ...
    auto altCursorPos = til::coord{ 5, 3 };
...
void ScreenBufferTests::EraseAllTests()
{
    ...
    stateMachine.ProcessString(L"foo");
    til::coord originalRelativePosition{ 3, 0 };
    VERIFY_ARE_EQUAL(si.GetViewport().Top(), 0);
...
void ScreenBufferTests::VtResizeDECCOLM()
{
    ...
    VERIFY_ARE_EQUAL(til::coord({ 0, 0 }), getRelativeCursorPosition());
    VERIFY_ARE_EQUAL(initialSbHeight, newSbHeight);
    VERIFY_ARE_EQUAL(initialViewHeight, newViewHeight);
...
void ScreenBufferTests::VtSoftResetCursorPosition()
{
    ...
    VERIFY_SUCCEEDED(si.SetViewportOrigin(true, til::coord({ 0, 0 }), true));
    ...
    VERIFY_ARE_EQUAL(til::coord({ 1, 1 }), cursor.GetPosition());
    ...
    VERIFY_ARE_EQUAL(til::coord({ 0, 0 }), cursor.GetPosition());
    ...
    stateMachine.ProcessString(L"\x1b[2;2H");
    VERIFY_ARE_EQUAL(til::coord({ 1, 5 }), cursor.GetPosition());
...
void ScreenBufferTests::VtScrollMarginsNewlineColor()
{
    ...
    Log::Comment(NoThrowString().Format(L"Make sure the viewport is at 0,0"));
    VERIFY_SUCCEEDED(si.SetViewportOrigin(true, til::coord({ 0, 0 }), true));
    cursor.SetPosition(til::coord({ 0, 0 }));
    ...
    for (int iteration = 0; iteration < 10; iteration++)
    {
        ...
        Log::Comment(NoThrowString().Format(
            L"Cursor=%s",
            VerifyOutputTraits<til::coord>::ToString(cursorPos).GetBuffer()));
        const auto viewport = si.GetViewport();
        Log::Comment(NoThrowString().Format(
            L"Viewport=%s",
            VerifyOutputTraits<til::small_rect>::ToString(viewport.ToInclusive()).GetBuffer()));
...
void ScreenBufferTests::VtNewlinePastViewport()
{
    ...
    Log::Comment(NoThrowString().Format(
        L"initialViewport=%s",
        VerifyOutputTraits<til::small_rect>::ToString(initialViewport.ToInclusive()).GetBuffer()));

    cursor.SetPosition(til::coord({ 0, initialViewport.BottomInclusive() }));
...
void ScreenBufferTests::VtNewlineOutsideMargins()
{
    ...
    stateMachine.ProcessString(L"\n");
    VERIFY_ARE_EQUAL(til::coord({ 0, viewportBottom + 1 }), cursor.GetPosition());
    VERIFY_ARE_EQUAL(til::coord({ 0, viewportTop + 1 }), si.GetViewport().Origin());

    Log::Comment(L"Reset viewport and apply DECSTBM margins");
    VERIFY_SUCCEEDED(si.SetViewportOrigin(true, til::coord({ 0, viewportTop }), true));
    ...
    cursor.SetPosition({ 0, viewportBottom });
    stateMachine.ProcessString(L"\n");
    VERIFY_ARE_EQUAL(til::coord({ 0, viewportBottom }), cursor.GetPosition());
    VERIFY_ARE_EQUAL(til::coord({ 0, viewportTop }), si.GetViewport().Origin());
}
...
void ScreenBufferTests::VtEraseAllPersistCursorFillColor()
{
    ...
    Log::Comment(NoThrowString().Format(
        L"new Viewport: %s",
        VerifyOutputTraits<til::small_rect>::ToString(newViewport.ToInclusive()).GetBuffer()));
    Log::Comment(NoThrowString().Format(
        L"Buffer Size: %s",
        VerifyOutputTraits<til::small_rect>::ToString(si.GetBufferSize().ToInclusive()).GetBuffer()));
...
void ScreenBufferTests::GetWordBoundary()
{
    ...
    const til::coord newBufferSize{ length, 10 };
    ...
    til::coord expectedFirst;
    til::coord expectedSecond{ 4, 0 };
...
void ScreenBufferTests::GetWordBoundaryTrimZeros(const bool on)
{
    ...
    til::coord expectedFirst;
    til::coord expectedSecond;
    std::pair<til::coord, til::coord> boundary;

    Log::Comment(L"Find lead with 000");
    expectedFirst = on ? til::coord{ 3, 0 } : til::coord{ 0, 0 };
    expectedSecond = til::coord{ 7, 0 };
    boundary = si.GetWordBoundary({ 0, 0 });
    VERIFY_ARE_EQUAL(expectedFirst, boundary.first);
    VERIFY_ARE_EQUAL(expectedSecond, boundary.second);

    Log::Comment(L"Find lead with 0x");
    expectedFirst = til::coord{ 8, 0 };
    expectedSecond = til::coord{ 14, 0 };
    boundary = si.GetWordBoundary({ 8, 0 });
    VERIFY_ARE_EQUAL(expectedFirst, boundary.first);
    VERIFY_ARE_EQUAL(expectedSecond, boundary.second);

    Log::Comment(L"Find lead with 0X");
    expectedFirst = til::coord{ 15, 0 };
    expectedSecond = til::coord{ 21, 0 };
    boundary = si.GetWordBoundary({ 15, 0 });
    VERIFY_ARE_EQUAL(expectedFirst, boundary.first);
    VERIFY_ARE_EQUAL(expectedSecond, boundary.second);

    Log::Comment(L"Find lead with 0n");
    expectedFirst = til::coord{ 22, 0 };
    expectedSecond = til::coord{ 28, 0 };
    boundary = si.GetWordBoundary({ 22, 0 });
    VERIFY_ARE_EQUAL(expectedFirst, boundary.first);
    VERIFY_ARE_EQUAL(expectedSecond, boundary.second);

    Log::Comment(L"Find lead with 0N");
    expectedFirst = on ? til::coord{ 30, 0 } : til::coord{ 29, 0 };
    expectedSecond = til::coord{ 35, 0 };
...
void ScreenBufferTests::TestAltBufferVtDispatching()
{
    ...
    if (VERIFY_IS_TRUE(NT_SUCCESS(Status)))
    {
        ...
        VERIFY_ARE_EQUAL(til::coord({ 0, 0 }), mainCursor.GetPosition());
        // recall: vt coordinates are (row, column), 1-indexed
        VERIFY_ARE_EQUAL(til::coord({ 5, 4 }), altCursor.GetPosition());
        ...
        VERIFY_ARE_EQUAL(til::coord({ 0, 0 }), mainCursor.GetPosition());
        VERIFY_ARE_EQUAL(til::coord({ 6, 4 }), altCursor.GetPosition());
...
void ScreenBufferTests::BackspaceDefaultAttrsInPrompt()
{
    ...
    til::coord expectedCursor{ 1, 1 }; // We're expecting y=1, because the 2J above
...
void ScreenBufferTests::SetGlobalColorTable()
{
    ...
    Log::Comment(NoThrowString().Format(L"Make sure the viewport is at 0,0"));
    VERIFY_SUCCEEDED(mainBuffer.SetViewportOrigin(true, til::coord({ 0, 0 }), true));
    ...
    stateMachine.ProcessString(L"X");
    VERIFY_ARE_EQUAL(til::coord({ 2, 0 }), altCursor.GetPosition());
    {
        const ROW& row = altBuffer.GetTextBuffer().GetRowByOffset(altCursor.GetPosition().y);
        const auto attrRow = &row.GetAttrRow();
        const std::vector<TextAttribute> attrs{ attrRow->begin(), attrRow->end() };
    ...
    Log::Comment(NoThrowString().Format(
        L"Print another X, both should be the new \"red\" color"));
    stateMachine.ProcessString(L"X");
    VERIFY_ARE_EQUAL(til::coord({ 2, 0 }), mainCursor.GetPosition());
    {
        const ROW& row = mainBuffer.GetTextBuffer().GetRowByOffset(mainCursor.GetPosition().y);
        const auto attrRow = &row.GetAttrRow();
        const std::vector<TextAttribute> attrs{ attrRow->begin(), attrRow->end() };
...
void ScreenBufferTests::DeleteCharsNearEndOfLine()
{
    ...
    VERIFY_ARE_EQUAL(til::coord({ mainView.Width() - 1, 0 }), mainCursor.GetPosition());
    ...
    Log::Comment(NoThrowString().Format(
        L"row_f=[%s]",
        tbi.GetRowByOffset(0).GetText().c_str()));
    VERIFY_ARE_EQUAL(til::coord({ mainView.Width() - static_cast<short>(dx), 0 }), mainCursor.GetPosition());
...
void ScreenBufferTests::DeleteCharsNearEndOfLineSimpleFirstCase()
{
    ...
    const til::coord newViewSize{ newBufferWidth, mainBuffer.GetViewport().Height() };
    ...
    VERIFY_ARE_EQUAL(til::coord({ 7, 0 }), mainCursor.GetPosition());
    ...
    VERIFY_ARE_EQUAL(til::coord({ 3, 0 }), mainCursor.GetPosition());
...
void ScreenBufferTests::DontResetColorsAboveVirtualBottom()
{
    ...
    Log::Comment(NoThrowString().Format(
        L"cursor=%s", VerifyOutputTraits<til::coord>::ToString(cursor.GetPosition()).GetBuffer()));
    Log::Comment(NoThrowString().Format(
        L"viewport=%s", VerifyOutputTraits<til::small_rect>::ToString(si.GetViewport().ToInclusive()).GetBuffer()));
...
template<class T>
void _FillLine(til::coord position, T fillContent, TextAttribute fillAttr)
{
    auto& gci = ServiceLocator::LocateGlobals().getConsoleInformation();
    auto& si = gci.GetActiveOutputBuffer().GetActiveBuffer();
...
template<class... T>
bool _ValidateLineContains(til::coord position, T&&... expectedContent)
{
    auto& gci = ServiceLocator::LocateGlobals().getConsoleInformation();
    auto& si = gci.GetActiveOutputBuffer().GetActiveBuffer();
...
void ScreenBufferTests::ScrollOperations()
{
    ...
    auto cursorPos = til::coord{ bufferWidth / 2, (viewportStart + viewportEnd) / 2 };
...
void ScreenBufferTests::EraseScrollbackTests()
{
    ...
    const auto cursorPos = til::coord{ centerX, centerY };
    ...
    const auto expectedOffset = til::coord{ 0, -viewport.Top() };
    const auto expectedViewport = Viewport::Offset(viewport, expectedOffset);
    const auto expectedCursorPos = til::coord{ cursorPos.x, cursorPos.y + expectedOffset.y };
...
void ScreenBufferTests::EraseTests()
{
    ...
    // 2. Cursor Line
    auto prefixPos = til::coord{ 0, cursorPos.y };
...
void ScreenBufferTests::LineFeedEscapeSequences()
{
...

    {
        ...
        cursor.SetPosition(til::coord{ initialX, initialY });
...
void ScreenBufferTests::SetOriginMode()
{
    ...
    VERIFY_ARE_EQUAL(til::coord({ 40, 12 }), cursor.GetPosition());
    Log::Comment(L"The cursor can be moved below the bottom margin.");
    stateMachine.ProcessString(L"\x1B[23;41H");
    VERIFY_ARE_EQUAL(til::coord({ 40, 22 }), cursor.GetPosition());
    ...
    cursor.SetPosition({ 40, 12 });
    stateMachine.ProcessString(L"\x1B[?6h");
    VERIFY_ARE_EQUAL(til::coord({ 0, 5 }), cursor.GetPosition());
    ...
    VERIFY_ARE_EQUAL(til::coord({ 40, 19 }), cursor.GetPosition());
...
void ScreenBufferTests::SetAutoWrapMode()
{
    ...
    VERIFY_ARE_EQUAL(til::coord({ 3, startLine + 1 }), cursor.GetPosition());
    ...
    VERIFY_ARE_EQUAL(til::coord({ 79, startLine }), cursor.GetPosition());
...
void ScreenBufferTests::HardResetBuffer()
{
    ...
    VERIFY_ARE_EQUAL(til::coord({ 0, 1 }), cursor.GetPosition());
    ...
    stateMachine.ProcessString(resetToInitialState);
    VERIFY_IS_TRUE(isBufferClear());
    VERIFY_ARE_EQUAL(til::coord({ 0, 0 }), viewport.Origin());
    VERIFY_ARE_EQUAL(til::coord({ 0, 0 }), cursor.GetPosition());
...
void ScreenBufferTests::RestoreDownAltBufferWithTerminalScrolling()
{
    ...
    if (VERIFY_IS_TRUE(NT_SUCCESS(siMain.UseAlternateScreenBuffer())))
    {
        ...
        const til::coord doubledSize{ originalSize.x * 2, originalSize.y * 2 };
...
void ScreenBufferTests::SnapCursorWithTerminalScrolling()
{
    ...
    Log::Comment(NoThrowString().Format(
        L"cursor=%s", VerifyOutputTraits<til::coord>::ToString(cursor.GetPosition()).GetBuffer()));
    Log::Comment(NoThrowString().Format(
        L"originalView=%s", VerifyOutputTraits<til::small_rect>::ToString(originalView.ToInclusive()).GetBuffer()));
    ...
    const til::coord secondWindowOrigin{ 0, 10 };
    ...
    Log::Comment(NoThrowString().Format(
        L"cursor=%s", VerifyOutputTraits<til::coord>::ToString(cursor.GetPosition()).GetBuffer()));
    Log::Comment(NoThrowString().Format(
        L"secondView=%s", VerifyOutputTraits<til::small_rect>::ToString(secondView.ToInclusive()).GetBuffer()));
    ...
    const til::coord thirdWindowOrigin{ 0, 2 };
    ...
    Log::Comment(NoThrowString().Format(
        L"cursor=%s", VerifyOutputTraits<til::coord>::ToString(cursor.GetPosition()).GetBuffer()));
    Log::Comment(NoThrowString().Format(
        L"thirdView=%s", VerifyOutputTraits<til::small_rect>::ToString(thirdView.ToInclusive()).GetBuffer()));
    ...
    Log::Comment(NoThrowString().Format(
        L"cursor=%s", VerifyOutputTraits<til::coord>::ToString(cursor.GetPosition()).GetBuffer()));
    Log::Comment(NoThrowString().Format(
        L"thirdView=%s", VerifyOutputTraits<til::small_rect>::ToString(fourthView.ToInclusive()).GetBuffer()));
...
void ScreenBufferTests::CursorNextPreviousLine()
{
    ...
    VERIFY_ARE_EQUAL(til::coord({ 0, 12 }), cursor.GetPosition());
...
void ScreenBufferTests::CursorPositionRelative()
{
    ...
    VERIFY_ARE_EQUAL(til::coord({ 25, 10 }), cursor.GetPosition());
    ...
    VERIFY_ARE_EQUAL(til::coord({ 20, 15 }), cursor.GetPosition());
    ...
    VERIFY_ARE_EQUAL(til::coord({ screenWidth - 1, 10 }), cursor.GetPosition());
    ...
    VERIFY_ARE_EQUAL(til::coord({ 20, screenHeight - 1 }), cursor.GetPosition());
}
...
void ScreenBufferTests::CursorSaveRestore()
{
    ...
    VERIFY_ARE_EQUAL(til::coord({ 20, 10 }), cursor.GetPosition());
    VERIFY_ARE_EQUAL(colorAttrs, si.GetAttributes());
    stateMachine.ProcessString(asciiText);
    VERIFY_IS_TRUE(_ValidateLineContains(til::coord({ 20, 10 }), graphicText, colorAttrs));
    ...
    stateMachine.ProcessString(asciiText);
    VERIFY_IS_TRUE(_ValidateLineContains(til::coord({ 0, 0 }), asciiText, defaultAttrs));
    ...
    VERIFY_ARE_EQUAL(til::coord({ 0, 9 }), cursor.GetPosition());
    ...
    stateMachine.ProcessString(L"\x1b[r");
    stateMachine.ProcessString(setDECOM);
    cursor.SetPosition(til::coord{ 5, 15 });
    // Save state.
    stateMachine.ProcessString(saveCursor);
    ...
    VERIFY_ARE_EQUAL(til::coord({ 5, 19 }), cursor.GetPosition());
    ...
    stateMachine.ProcessString(restoreCursor);
    // Verify Y position is clamped inside the top margin
    VERIFY_ARE_EQUAL(til::coord({ 5, 9 }), cursor.GetPosition());
...
void ScreenBufferTests::ScreenAlignmentPattern()
{
    ...
    auto homePosition = til::coord{ 0, viewportStart };
...
void ScreenBufferTests::UpdateVirtualBottomWhenCursorMovesBelowIt()
{
    ...
    const auto initialCursorPos = til::coord{ 0, initialVirtualBottom };
    ...
    const auto initialOrigin = til::coord{ 0, si.GetViewport().Top() + si.GetViewport().Height() / 2 };
    ...
    const auto newCursorPos = til::coord{ initialCursorPos.x, initialCursorPos.y + 10 };
...
void ScreenBufferTests::RetainHorizontalOffsetWhenMovingToBottom()
{
    ...
    auto initialSize = til::coord{ CommonState::s_csWindowWidth / 2, CommonState::s_csWindowHeight };
    si.SetViewportSize(&initialSize);

    Log::Comment(L"Offset the viewport both vertically and horizontally");
    auto initialOrigin = til::coord{ 10, 20 };
