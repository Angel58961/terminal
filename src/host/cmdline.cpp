...
void SetCurrentCommandLine(COOKED_READ_DATA& cookedReadData, _In_ int Index) // index, not command number
{
...
void CommandLine::_setPromptToNewestCommand(COOKED_READ_DATA& cookedReadData)
{
    ...
    if (cookedReadData.HasHistory() && cookedReadData.History().GetNumberOfCommands())
    {
        const auto commandNumber = cookedReadData.History().GetNumberOfCommands() - 1;
        THROW_IF_FAILED(cookedReadData.History().RetrieveNth(commandNumber,
                                                             cookedReadData.SpanWholeBuffer(),
                                                             cookedReadData.BytesRead()));
        FAIL_FAST_IF(!(cookedReadData.BufferStartPtr() == cookedReadData.BufferCurrentPtr()));
...
til::coord CommandLine::_deletePromptBeforeCursor(COOKED_READ_DATA& cookedReadData) noexcept
{
    DeleteCommandLine(cookedReadData, false);
    cookedReadData.BytesRead() -= cookedReadData.InsertionPoint() * sizeof(WCHAR);
...
// Return Value:
// - The new cursor position
til::coord CommandLine::_moveCursorToEndOfPrompt(COOKED_READ_DATA& cookedReadData) noexcept
{
...
// Return Value:
// - The new cursor position
til::coord CommandLine::_moveCursorToStartOfPrompt(COOKED_READ_DATA& cookedReadData) noexcept
{
    cookedReadData.InsertionPoint() = 0;
    cookedReadData.SetBufferCurrentPtr(cookedReadData.BufferStartPtr());
    return cookedReadData.OriginalCursorPosition();
...
// Return Value:
// - New cursor position
til::coord CommandLine::_moveCursorLeftByWord(COOKED_READ_DATA& cookedReadData) noexcept
{
    PWCHAR LastWord;
...
// Return Value:
// - New cursor position
til::coord CommandLine::_moveCursorLeft(COOKED_READ_DATA& cookedReadData)
{
    auto cursorPosition = cookedReadData.ScreenInfo().GetTextBuffer().GetCursor().GetPosition();
    if (cookedReadData.BufferCurrentPtr() != cookedReadData.BufferStartPtr())
    {
...
// Return Value:
// - The new cursor position
til::coord CommandLine::_moveCursorRightByWord(COOKED_READ_DATA& cookedReadData) noexcept
{
    auto cursorPosition = cookedReadData.ScreenInfo().GetTextBuffer().GetCursor().GetPosition();
...
// Return Value:
// - The new cursor position
til::coord CommandLine::_moveCursorRight(COOKED_READ_DATA& cookedReadData) noexcept
{
    auto cursorPosition = cookedReadData.ScreenInfo().GetTextBuffer().GetCursor().GetPosition();
    const auto sScreenBufferSizeX = cookedReadData.ScreenInfo().GetBufferSize().Width();
...
// Return Value:
// - The new cursor position
til::coord CommandLine::_cycleMatchingCommandHistoryToPrompt(COOKED_READ_DATA& cookedReadData)
{
    auto cursorPosition = cookedReadData.ScreenInfo().GetTextBuffer().GetCursor().GetPosition();
    if (cookedReadData.HasHistory())
    {
...
// Return Value:
// - The new cursor position
til::coord CommandLine::DeleteFromRightOfCursor(COOKED_READ_DATA& cookedReadData) noexcept
{
    // save cursor position
    auto cursorPosition = cookedReadData.ScreenInfo().GetTextBuffer().GetCursor().GetPosition();
