...
void ScreenBufferRenderTarget::TriggerRedraw(const til::coord* const pcoord)
{
    auto* pRenderer = ServiceLocator::LocateGlobals().pRender;
    const auto* pActive = &ServiceLocator::LocateGlobals().getConsoleInformation().GetActiveOutputBuffer().GetActiveBuffer();
    if (pRenderer != nullptr && pActive == &_owner)
...
void ScreenBufferRenderTarget::TriggerRedrawCursor(const til::coord* const pcoord)
{
    auto* pRenderer = ServiceLocator::LocateGlobals().pRender;
    const auto* pActive = &ServiceLocator::LocateGlobals().getConsoleInformation().GetActiveOutputBuffer().GetActiveBuffer();
    if (pRenderer != nullptr && pActive == &_owner)
...
void ScreenBufferRenderTarget::TriggerScroll(const til::coord* const pcoordDelta)
{
    auto* pRenderer = ServiceLocator::LocateGlobals().pRender;
    const auto* pActive = &ServiceLocator::LocateGlobals().getConsoleInformation().GetActiveOutputBuffer().GetActiveBuffer();
    if (pRenderer != nullptr && pActive == &_owner)
