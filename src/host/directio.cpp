                                                           ...
                                                           Microsoft::Console::Types::Viewport& readRectangle) noexcept
{
    try
    {
        ...
        clip.right = std::min<decltype(clip.right)>(clip.right, storageSize.x);
        clip.bottom = std::min<decltype(clip.bottom)>(clip.bottom, storageSize.y);
        ...
        clip.left = std::max<decltype(clip.left)>(clip.left, 0);
        clip.top = std::max<decltype(clip.top)>(clip.top, 0);
