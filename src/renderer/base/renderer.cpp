...
// Return Value:
// - <none>
void Renderer::TriggerRedraw(const til::coord* const pcoord)
{
...
// Return Value:
// - <none>
void Renderer::TriggerRedrawCursor(const til::coord* const pcoord)
{
    ...
    if (buffer.GetSize().IsInBounds(*pcoord))
    {
        ...
        const til::small_rect cursorRect{ pcoord->x, pcoord->y, pcoord->x + cursorWidth - 1, pcoord->y };
...
bool Renderer::_CheckViewportAndScroll()
{
    ...
    til::coord coordDelta;
    coordDelta.x = SHORT(srOldViewport.left) - SHORT(srNewViewport.left);
    coordDelta.y = SHORT(srOldViewport.top) - SHORT(srNewViewport.top);
...
// Return Value:
// - <none>
void Renderer::TriggerScroll(const til::coord* const pcoordDelta)
{
    FOREACH_ENGINE(pEngine)
    {
...
void Renderer::_PaintBufferOutput(_In_ IRenderEngine* const pEngine)
{
    ...
    for (const auto& dirtyRect : dirtyAreas)
    {
        ...
        for (auto row = redraw.Top(); row < redraw.BottomExclusive(); row++)
        {
            ...
            const auto screenLine = til::small_rect{ redraw.Left(), row, redraw.RightInclusive(), row };
            ...
            const auto screenPosition = bufferLine.Origin() - til::coord{ 0, view.Top() };
                                        ...
                                        const bool lineWrapped)
{
    ...
    if (it)
    {
        ...
        while (it)
        {
            ...
            do
            {
                til::coord thisPoint{ screenPoint.x + cols, screenPoint.y };
                             ...
                             const RenderOverlay& overlay)
{
    try
    {
        ...
        for (const auto& rect : dirtyAreas)
        {
            ...
            auto srDirty = rect.to_rect<til::small_rect>();
            srDirty.bottom++;
            srDirty.right++;
            ...
            if (viewConv.TrimToViewport(&srDirty))
            {
                ...
                {
                    const til::coord target{ viewDirty.Left(), iRow };
...
std::vector<til::small_rect> Renderer::_GetSelectionRects() const
{
    const auto& buffer = _pData->GetTextBuffer();
