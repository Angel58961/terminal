...
[[nodiscard]] HRESULT AtlasEngine::PaintBufferLine(const gsl::span<const Cluster> clusters, const til::coord coord, const bool fTrimLeft, const bool lineWrapped) noexcept
try
{
...
[[nodiscard]] HRESULT AtlasEngine::PaintBufferGridLines(const GridLineSet lines, const COLORREF color, const size_t cchLine, const til::coord coordTarget) noexcept
{
    return S_OK;
}

[[nodiscard]] HRESULT AtlasEngine::PaintSelection(til::small_rect rect) noexcept
try
{
    ...
    _flushBufferLine();
    {
        const auto left = gsl::narrow_cast<u16>(clamp<int>(rect.left, 0, _r.cellCount.x));
        const auto top = gsl::narrow_cast<u16>(clamp<int>(rect.top, 0, _r.cellCount.y));
        const auto right = gsl::narrow_cast<u16>(clamp<int>(rect.right, 0, _r.cellCount.x));
        const auto bottom = gsl::narrow_cast<u16>(clamp<int>(rect.bottom, 0, _r.cellCount.y));
        _setCellFlags({left, top, right, bottom}, CellFlags::Selected, CellFlags::Selected);
    }
    return S_OK;
}
CATCH_RETURN()
...
try
{
    ...
    if (options.isOn)
    {
        ...
        const auto x = gsl::narrow_cast<u16>(std::min<int>(point.x, _r.cellCount.x - 1));
        const auto y = gsl::narrow_cast<u16>(std::min<int>(point.y, _r.cellCount.y - 1));
        const u16 right = x + 1 + (options.fIsDoubleWidth & (options.cursorType != CursorType::VerticalBar));
        const u16 bottom = y + 1;
...
void AtlasEngine::_setCellFlags(u16r coords, CellFlags mask, CellFlags bits) noexcept
{
    ...
    auto row = &_r.cells[coords.top * stride + coords.left];
