...
[[nodiscard]] HRESULT AtlasEngine::Invalidate(const til::small_rect* const psrRegion) noexcept
{
    // We don't assert that psrRegion->bottom <= _api.cellCount.y, as the
    // cellCount is only updated in BeginDraw() if we're drawing for a HWND.
    assert(psrRegion->top < psrRegion->bottom && psrRegion->top >= 0);
...
[[nodiscard]] HRESULT AtlasEngine::InvalidateCursor(const til::small_rect* const psrRegion) noexcept
{
...
[[nodiscard]] HRESULT AtlasEngine::InvalidateSystem(const RECT* const prcDirtyClient) noexcept
{
    ...
    // BeginPaint() protects against invalid out of bounds numbers.
    til::small_rect rect;
    rect.top = gsl::narrow_cast<decltype(rect.top)>(top);
    rect.bottom = gsl::narrow_cast<decltype(rect.bottom)>(bottom);
    return Invalidate(&rect);
}

[[nodiscard]] HRESULT AtlasEngine::InvalidateSelection(const std::vector<til::small_rect>& rectangles) noexcept
{
    for (const auto& rect : rectangles)
    {
        //assert(rect.top <= rect.bottom && rect.top >= 0);
        // BeginPaint() protects against invalid out of bounds numbers.
...
[[nodiscard]] HRESULT AtlasEngine::InvalidateScroll(const til::coord* const pcoordDelta) noexcept
{
...
[[nodiscard]] HRESULT AtlasEngine::UpdateViewport(const til::small_rect srNewViewport) noexcept
{
    return S_OK;
}
...
[[nodiscard]] HRESULT AtlasEngine::GetFontSize(_Out_ til::coord* const pFontSize) noexcept
{
...
[[nodiscard]] Microsoft::Console::Types::Viewport AtlasEngine::GetViewportInCharacters(const Types::Viewport& viewInPixels) const noexcept
{
    ...
    return Types::Viewport::FromDimensions(viewInPixels.Origin(), til::coord{ viewInPixels.Width() / _api.fontMetrics.cellSize.x, viewInPixels.Height() / _api.fontMetrics.cellSize.y });
}
...
[[nodiscard]] Microsoft::Console::Types::Viewport AtlasEngine::GetViewportInPixels(const Types::Viewport& viewInCharacters) const noexcept
{
    assert(_api.fontMetrics.cellSize.x != 0);
    assert(_api.fontMetrics.cellSize.y != 0);
    return Types::Viewport::FromDimensions(viewInCharacters.Origin(), til::coord{ viewInCharacters.Width() * _api.fontMetrics.cellSize.x, viewInCharacters.Height() * _api.fontMetrics.cellSize.y });
}
...
void AtlasEngine::_resolveFontMetrics(const FontInfoDesired& fontInfoDesired, FontInfo& fontInfo, FontMetrics* fontMetrics) const
{
    ...
    const auto fontSizeInPx = std::ceil(gsl::narrow_cast<unsigned int>(requestedSize.y) / 72.0 * _api.dpi);
