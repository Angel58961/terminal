...
[[nodiscard]] HRESULT VtEngine::PaintBufferGridLines(const GridLineSet /*lines*/,
                                                     ...
                                                     const til::coord /*coordTarget*/) noexcept
{
    return S_OK;
}
...
// Return Value:
// - S_OK
[[nodiscard]] HRESULT VtEngine::PaintSelection(const til::small_rect /*rect*/) noexcept
{
    return S_OK;
}
                                                      ...
                                                      const til::coord coord) noexcept
{
    try
    {
        ...
        RETURN_IF_FAILED(SizeTToInt(totalWidth, &_lastText.x));
                                                     ...
                                                     const bool lineWrapped) noexcept
{
    ...
    {
        _bufferLine.append(cluster.GetText());
        RETURN_IF_FAILED(IntAdd(totalWidth, static_cast<short>(cluster.GetColumns()), &totalWidth));
    }
    ...
    RETURN_IF_FAILED(SizeTToInt(numSpaces, &sNumSpaces));
