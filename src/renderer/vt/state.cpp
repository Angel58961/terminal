...
const til::coord VtEngine::INVALID_COORDS = { -1, -1 };
...
// Return Value:
// - HRESULT S_OK
[[nodiscard]] HRESULT VtEngine::UpdateViewport(const til::small_rect srNewViewport) noexcept
{
    HRESULT hr = S_OK;
...
// Return Value:
// - S_FALSE: This is unsupported by the VT Renderer and should use another engine's value.
[[nodiscard]] HRESULT VtEngine::GetFontSize(_Out_ til::coord* const pFontSize) noexcept
{
    *pFontSize = til::coord({ 1, 1 });
    return S_FALSE;
}
...
// Return Value:
// - S_OK
[[nodiscard]] HRESULT VtEngine::InheritCursor(const til::coord coordCursor) noexcept
{
