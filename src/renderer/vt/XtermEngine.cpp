...
// Return Value:
// - S_OK if we succeeded, else an appropriate HRESULT for failing to allocate or write.
[[nodiscard]] HRESULT XtermEngine::_MoveCursor(til::coord const coord) noexcept
{
    HRESULT hr = S_OK;
...
try
{
    ...
    const auto dy = _scrollDelta.y<til::CoordType>();
...
// Arguments:
// - pcoordDelta - Pointer to character dimension (til::coord) of the distance the
//      console would like us to move while scrolling.
// Return Value:
// - S_OK if we succeeded, else an appropriate HRESULT for safemath failure
[[nodiscard]] HRESULT XtermEngine::InvalidateScroll(const til::coord* const pcoordDelta) noexcept
try
{
